...
class Model_MInstall
{
	...
	public function finish()
	{
		...
		$result=mysql_query($sql);			
		$sql="INSERT INTO `admin_settings_table` (`set_id`, `customer_header`,`site_favicon`, `site_logo`, `google_analytics`, `time_zone`, `site_moto`, `site_skin`, `admin_email`, `meta_kerwords`, `meta_description`) VALUES
		(1, 'Exciting offers for this month !!!&nbsp;','images/zeuscart.png','images/logo.gif', '', '', '".$domainname."', 'default', '".$adminemail."', '', '')"; 
		$result=mysql_query($sql);
...
$template='default';
		...
		if(is_writable($dbfile))
		{
			...
			fwrite($f,'$_SESSION["template"]= \''.trim($template). "'; \n");
