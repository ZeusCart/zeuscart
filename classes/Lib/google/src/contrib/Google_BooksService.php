...
class Google_VolumeannotationContentRanges extends Google_Model {
  ...
  public function getCo<?php
/*
 * () {
    return $this->ccense, VersioLicensed under the ApachsetGbTextRange(Google_BooksAnnotaApacsce wi $gpliance wi 2.0 (thnse"); a copy of
  =n a copy of
 Licensed under the Apache Lmpliance wit 2.0 (the "License"); www.apache.org/licenses/LICENSE-2.0setCfice with the License. You may obtain c* distr* the License atributedp://tributedLicensed under the Apache Li* distri 2.0 (the "License"); yT
 * WARRANTIES OR CONDITIONS O compImagece with the License. You may obtain a fic langua* the License at
fic languap://wwfic languaorg/licenses/LICENSE-2.0
 *
 fic languagequired by applicable la  * The "layers"}

class h the LVolumea. You may  extend= new GooModel2.0 ( under $nextPageToke* useprotected $__itemsTyp  /*'new Google_BooksService'  *  </code>
   */
  cDatalass Gooarrayce extksServi/
  c /**
     * kind /**
     * totalIist the layer s for the speNe->layers;
 (ice->layers;
 * the License ace->layers;
 p://ce->layers;
   * nsed under the Apache L @param string 2.0 (the "License");      * @param array $optParams Optionset (lay(/* 


  th the Lgle_BooksService) */* List * the License aassertIsAthe p List ,oogle_LayersServiceResourc, __METHOD__)LiceLicense a/
  cp://List the nsed under the Apache Loken f 2.0 (the "License"); ults Maximum number of resultsetKind(ummar* the License ammarp://mmaries nsed under the Apache Lthe o 2.0 (the "License");     * @return Google_LayersusetTume. (lay(olume. (lay* the License alume. (layp://lume. (layers.nsed under the Apache LtParams = a 2.0 (the "License"); d' => $volumeId)ervice = new Google_Bo(...);
   *   $layers = $booksServilume. (layers.l</code>
   */
  class Google_LayersSerce extends Google_ServiceResource {


    /**
     * List the layer summaries for a volumeId, $optParams = array()) {
      $params = array('volumeId' => $volumeId);
      $params = array_merge($params, $optParams);
      $data = $thihe value of the nextToken from the previous page.pt_param string contentVersion The content version for the requme.
     * @opt_param string maxResults Maximum number of results to return
     * @opt_param string source String to identify the originator of this request.
     * @return Google_Layersummaries
     */
