...
class Core_CUserRegistration
{
	...
	*/
	...
	function showHeaderMenu()
	{
		...
		$sqlCms="SELECT * FROM cms_table WHERE cms_page_status='1'";
		$objCms=new Bin_Query();
		$objCms->executeQuery($sqlCms);
		$recordsCms=$objCms->records;
		...
		{
			$output = Display_DUserRegistration::showHeaderMenu($query->records,$recordsCms);
		}
		else
			$output='';
		return $output;
	 ...
	 */
	 function googleautoRegister($me)
   	...
   	function getDynamicCms()
   	{
   		...
   		$sql="SELECT * FROM cms_table WHERE cms_page_alias='".trim($_GET['cat'])."'";
   		$obj=new Bin_Query();
   		$obj->executeQuery($sql);
   		$records=$obj->records[0];
