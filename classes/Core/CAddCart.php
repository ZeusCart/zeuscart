...
class Core_CAddCart 
{
			...
			if($_GET['prodid']!='')
			{
				if($_SESSION['user_id']!='')
				{
					...
					if($cartid!=0) // if cart available for the user
					{
						...
						else
						{
							...
							if($soh!=0)
							{
								...
								$sql ="insert into shopping_cart_products_table (cart_id,product_id , product_qty , date_added ,product_unit_price,add_items) values (".$cartid.','. $_GET['prodid'].",1,'".date('Y-m-d')."',".$originalprice.",'2')";
								$query->updateQuery($sql);
							}
							else
							{
					...
					else // if cart is not available for the user  cnt ==0
					{
						...
						$cartid=$this->getCartIdOfUser();
						$sql1 ="insert into shopping_cart_products_table (cart_id,product_id , product_qty , date_added,product_unit_price,add_items) values (".$cartid.','.$_GET['prodid'].",1,'".date('Y-m-d')."',".$originalprice.",'2')"; 
				...
				if($_SESSION['user_id']!='')
				{
					...
					if( $cartid!=0) // if cart available for the user
					{
						...
						else
						{
							...
							if($soh!=0)
							{
								$sql ="insert into shopping_cart_products_table (cart_id,product_id , product_qty , date_added ,product_unit_price,shipping_cost,gift_product,variation_id,add_items) values (".$cartid.','. $_GET['prodid'].",".$qtyfrmproduct.",'".date('Y-m-d')."',".$msrp.",".$shippingcost.",'".$_GET['vid']."',".(int)$_POST['variations'].",'2')"; 
								$query->updateQuery($sql);
							}	
						}
					}
	...
	function deleteCart()
	{
		...
		if($_SESSION['user_id']!='' && $_GET['from']=='order') 
		{ 
			...
			if($cartid !='')
			{
				...
				for($i=0;$i<=$cnt;$i++)
				{
					...
					if($qty[$i]['product_qty']==1)
					{
						 ...
						 $sql='SELECT pt.title, pt.model, pt.product_id,pt.digital, pt.brand, shopping_cart_products_table.shipping_cost AS shipingamount, pt.sku, pt.msrp as msrp1,shopping_cart_products_table.product_unit_price AS msrp, pt.image, pt.thumb_image, pinv.soh,shopping_cart_products_table.id, shopping_cart_products_table. * , shopping_cart_table. *
						FROM (
						products_table pt
						INNER JOIN shopping_cart_products_table ON pt.product_id = shopping_cart_products_table.product_id
						)
					...
					else
					{
						...
						$query = new Bin_Query();
						$sql='SELECT pt.title, pt.model, pt.product_id, pt.brand,pt.digital, shopping_cart_products_table.shipping_cost as shipingamount, pt.sku, shopping_cart_products_table.product_unit_price AS msrp, pt.msrp as msrp1, pt.image, pt.thumb_image, pinv.soh,shopping_cart_products_table.id,shopping_cart_products_table. * , shopping_cart_table. *
						FROM (
						products_table pt
						INNER JOIN shopping_cart_products_table ON pt.product_id = shopping_cart_products_table.product_id
						)
			...
			if(isset($_SESSION['mycart']) && $_SESSION['mycart']!='' )
			{
					...
					else
					{
						...
						foreach ($_SESSION['mycart'] as $key=>$val)
						{
							...
							{
								$sqlinsert ="insert into shopping_cart_products_table (cart_id,product_id,product_qty, date_added,product_unit_price,shipping_cost,gift_product,variation_id,original_price,add_items) values ('".$cartid."','".$val['product_id']."','".$val['qty']."','".date('Y-m-d')."','".$msrp."','".$query->records[0]['shipingamount']."','".$val['gift']."','".$val['variation_id']."','".$original_price."','1')"; 
					...
					foreach ($_SESSION['mycart'] as $key=>$val)
					{
						...
						$sql='SELECT pt.title, pt.model, pt.product_id, pt.brand, pt.shipping_cost AS shipingamount, pt.sku, pt.msrp, pt.image, pt.thumb_image, pinv.soh,scp.cart_id,scp.product_id,scp. product_unit_price,scp.variation_id,scp.product_qty ,scp.id,scp.add_items 
						FROM products_table pt
						LEFT JOIN product_inventory_table AS pinv ON pt.product_id = pinv.product_id
						LEFT JOIN shopping_cart_products_table AS scp ON scp.product_id = pinv.product_id	
						WHERE pt.product_id ='.$val['product_id'].' AND scp.cart_id='.$cartid.' AND scp.add_items="1"' ;   
...
.'
	...
	function countCart()
	{
		...
		if(isset($_SESSION['mycart']) )
		{
			...
			if($objCheck->executeQuery($sqlCheck))	
			{
				$sqlShop="SELECT a.*,b.* FROM shopping_cart_table AS a  JOIN shopping_cart_products_table AS b ON a.cart_id=b.cart_id WHERE a.user_id='". $_SESSION['user_id']."' AND add_items='1'"; 
		...
		else if(isset($_SESSION['user_id']) && $_SESSION['mycart']=='')
		{
