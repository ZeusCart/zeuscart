...
// 			}
			...
			$results=str_replace('-',' ',$results);
			 ...
			 if(count($results) > 1){
				...
				$sqlBase='SELECT * from category_table WHERE  category_id="'.trim($results[0]).'"';
	  		...
	  		$last=end($results);
			$sql="SELECT * from category_table WHERE category_id='".trim($last)."' ".$sql.""; 
			$obj=new Bin_Query();
			$obj->executeQuery($sql);
			...
			//product selection
			 $sqlpro="SELECT a.product_id,a.alias, a.title, a.thumb_image,a.image,a.large_image_path,a.product_status,a.category_id,a.gift,a.description ,a.msrp,a.intro_date,b.soh FROM products_table a INNER JOIN	product_inventory_table b ON a.product_id=b.product_id   and a.status=1 and a.gift!='1' and product_status!='3' and a.category_id   IN ($categoryid) GROUP BY a.product_id";  
	...
	function viewGiftProducts()
	{
		...
		$total = 0;
		 ...
		 $sqlpro="SELECT a.product_id, a.title,a.large_image_path,a.thumb_image,a.image,a.product_status,a.category_id,a.gift,a.description,a.msrp,a.intro_date,b.soh  as rating	FROM products_table a INNER JOIN	product_inventory_table b ON a.product_id=b.product_id  WHERE a.intro_date <= '".date('Y-m-d')."' and a.status=1 and a.gift='1' and product_status!='3' GROUP BY a.product_id";
	...
	function getTitle()
	{
		...
		$sqlParent="SELECT * FROM category_table WHERE category_alias='".$parentcat."' AND category_status =1";	
		...
		$category=str_replace('-',' ',$category);
	 	$sql="SELECT * FROM  category_table  WHERE category_alias='".$category."' AND FIND_IN_SET($parent_id,subcat_path) AND category_status =1";
		$obj=new Bin_Query();
		$obj->executeQuery($sql);	
