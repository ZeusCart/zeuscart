...
class Core_CCurrencySettings
{
	...
	function getDefaultCurrency()
	{
		...
		Core_CLanguage::setDisplay('DCurrencySettings.php');

		
		if($_SESSION['currencysetting']['selected_currency_id']=='')
		{
			$sql="SELECT currency_name,currency_code,currency_tocken FROM currency_master_table WHERE id=1 AND default_currency=1 AND status='1'"; 
			$qry=new Bin_Query();
	...
	function displayEnabledCurrencies()
	{
		...
		$sql="SELECT a.id,a.currency_name,a.status,a.currency_code,a.currency_tocken,a.default_currency,b.cou_name as country_name FROM currency_master_table a, country_table b WHERE a.status=1 AND a.country_code=b.cou_code";
		$obj=new Bin_Query();
