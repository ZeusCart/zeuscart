...
class Core_CHome
{
	...
	function pageTitle()
	{
		...
		Core_CLanguage::setDisplay('DHome.php');
   		...
   		$sqlSettings= "SELECT * FROM admin_settings_table where set_id= '1'";
		$querySettings = new Bin_Query();
		$querySettings->executeQuery($sqlSettings);	
		$records_settings=$querySettings->records;
		if($_GET['prodid']!='')
		{
			$sql= "SELECT product_id,title,meta_desc,meta_keywords FROM products_table where  	product_id= '".$_GET['prodid']."' and status=1"; 
			$query = new Bin_Query();
			$query->executeQuery($sql);		
			return  Display_DHome::pageTitle($query->records,$records_settings);
		}
		elseif($_GET['id']!='' && $_GET['do']=='category')
		{
			...
			$query = new Bin_Query();
			$query->executeQuery($sql);		
			return  Display_DHome::pageCategory($query->records,$records_settings);
		}
		else
		{			
			return  Display_DHome::siteMetaInformation($records_settings);
		}
