...
class Display_DUserRegistration
{
	...
	* @return string
 	*/
	function showHeaderMenu($arr,$recordsCms)
	{
		...
		$output.=self::getDynamicCmsPage($recordsCms);
	...
	/**
	 * Function generates an drop down list for the dynamic cms 
	 * 
	 * 
	 * @return array
	 */	
	function getDynamicCmsPage($recordsCms)
	{
		$cnt= count($recordsCms);
		if($recordsCms>0)
		{
			for($i=0;$i<$cnt;$i++)
			{
				...
				$sluggable = $recordsCms[$i]['cms_page_alias'];

				$output.='<li><a href="'.$_SESSION['base_url'].'/index.php/cms/'.$sluggable.'.html" class="arrow">'.$recordsCms[$i]['cms_page_title'].'</a>
			...
			while($rowSubFamilies = mysql_fetch_assoc($resultSubFamilies )) {
				...
				if($rowSubFamilies['category_id'])
					...
					if($level=='1')
					{
						...
						$output.='<div class="col-1"><ol><a href="'.$_SESSION['base_url'].'/index.php/'.$categoryname.'.html"><h4>'.$rowSubFamilies['category_name'].'</h4></a>';	
					...
					else
					{
						...
						$output.='
						<li ><a href="'.$_SESSION['base_url'].'/index.php/'.$categoryname.'.html"> '.$rowSubFamilies['category_name'].'</a></li>';
