		...
		<table class="rt cf" id="rt1">
			...
			if(count($arr)>0)  
			{
				for($i=0;$i<count($arr);$i++)			  
				{
					...
					<td> '.$arr[$i]['orders_status_name'].'</td>
		...
		<thead class="cf">
			 ...
			 if(count($arr)>0)	  
			{
				for($i=0;$i<count($arr);$i++)
				{
					...
					<td>'.Core_CLanguage::_(ORDER_STATUS_NAME.$arr[$i]['orders_status']).'</td>
	...
	function showOrderDetails($arr)
	{
		...
		<div class="myacc_detail">
			...
			for($i=0;$i<count($arr);$i++)
			{
				...
				<tr>
				<td>'.Core_CLanguage::_(TAX_APPLIED).'</td>
				<td><span class="label label-inverse">'.$arr[0]['currency_tocken'].'&nbsp;'.number_format($order_tax,2).'</span></td>
			</tr>
