...
class Display_DHome
{
			...
			if(count($records)>0)
			{
				for($i=0;$i<count($records);$i++)
				{
					...
					$output.='<li><form name="product" method="post" action="'.$_SESSION['base_url'].'/index.php?do=addtocart&prodid='.$records[$i]['product_id'].'">
						...
						$output.='<button class="add_btn" type="submit" ><p style="padding-left:40px">'.Core_CLanguage::_(ADD_TO_CART).'</p></button>';
	...
	* @name pageInfo
	* @param mixed $arr
	* @param array $records_settings
	* @return string
 	*/
	function pageTitle($arr,$records_settings)
	{
	...
	* @return string
 	*/
	function pageCategory($arr,$records_settings)
	{
