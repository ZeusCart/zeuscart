...
// 		$output.='</ul>';
		...
		for($i=0;$i<count($arr);$i++)
		{
			...
			$output.=' <li><a href="'.$_SESSION['base_url'].'/index.php/'.$sluggable.'.html">'.$arr[$i]['category_name'].'</a>';
	...
	function getSubFamilies($level,$id,$k) {
		...
		if (mysql_num_rows($resultSubFamilies) > 0 ) {
			...
			while($rowSubFamilies = mysql_fetch_assoc($resultSubFamilies )) {
				...
				<li ><a href="'.$_SESSION['base_url'].'/index.php/'.$categoryname.'.html"> '.$rowSubFamilies['category_name'].'</a>';
	...
	function getCountSubFamilies($id)
	{
		...
		$sql ="SELECT *  FROM  category_table WHERE FIND_IN_SET( ".$id.",subcat_path) AND category_status =1"; 
		$obj=new Bin_Query();
		$obj->executeQuery($sql);
