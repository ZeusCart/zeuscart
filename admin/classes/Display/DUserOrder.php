...
// 	}
	...
	function listOrder($arr)
	{
		...
		if(count($arr)>0)
		{
			...
			<input type="hidden" name="order_total" id="order_total" value="'.$grandTotal.'"></td></tr>
			<td colspan=5  align=right><b>Shipping Cost</b></td><td align="right" >'.$_SESSION['currency']['currency_tocken'].number_format($shiptotal,2).'
			<input type="hidden" name="shipping" value="'.$shiptotal.'"></td>	</tr>
			<td colspan=3  align=left><b>Discount </b>&nbsp;<input type="text" style="width:30%" name="discountrate"  id="discountrate">&nbsp;<select style="width:30%" name="discount" id="discount"><option value="">Select Discount Method</option><option value="flat">Flat rate</option><option value="percentage">Percentage</option></select>&nbsp;<a href="javascript:void(0);" onclick="coupondiscount('.$grandTotal.');"><input type="button" class="all_bttn" value="Apply" id="add" name="Apply"></a></td>
