...
class Core_CSubAdminManagement
{
  ...
  //$sql="update subadmin_table set subadmin_name='".$name."',subadmin_password='".base64_encode($password)."',subadmin_email_id='".$email."', subadmin_status=".$status." where subadmin_id=".$id;
		$sql="update subadmin_table set subadmin_password='".md5($password)."',subadmin_email_id='".$email."', subadmin_status=".$status." where subadmin_id=".$id;
	...
	function insertSubAdmin()
	{
	 ...
	 //  if($name!='' && $email!='' && $password!='')
	   //{
		$sql="insert into subadmin_table (subadmin_name,subadmin_password,subadmin_email_id,subadmin_status)values('".$name."','".md5($password)."','".$email."',".$status.")";
		$obj=new Bin_Query();
