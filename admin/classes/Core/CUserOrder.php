...
// 			}
	...
	function createOrder()
	{
		...
		if(trim($_POST['discount'])!='')
		{
			...
			if(trim($_POST['discount'])=='flat')
			{

				 $discount_rate=trim($_POST['discountrate']);
			}
			else if(trim($_POST['discount'])=='percentage')
			{
				 $discount_rate=$order_total * (trim($_POST['discountrate']/100));

			}
		 	 $order_total=$_POST['order_total']-$discount_rate;
		}
		...
		if((int)$customers_id!=0 && $customers_id!='' && count($_SESSION['ProductDetails'])>0)
		{
				...
				order_tax, ipn_id, ip_address,shipment_id_selected,discount)
				values
				('".$customers_id."','".$shipping_name."','".$shipping_company."','".$shipping_street_address."','".$shipping_suburb."','".$shipping_city."','".$shipping_postcode."','".$shipping_state."','".$shipping_country."','".$billing_name."','".$billing_company."','".$billing_street_address."','".$billing_suburb."','".$billing_city."','".$billing_postcode."','".$billing_state."','".$billing_country."','".$payment_method."','".$shipping_method."','".$coupon_code."','".$date_purchased."','".$orders_date_closed."','".$orders_status."','".$order_total."','".$order_tax."','".$paypal_ipn_id."','".$ip_address."','1','".$discount_rate."')";
				$obj=new Bin_Query();
	...
	function showUserAddressDetails()
	{
		...
		$sqlUserDetails="select first_name,company,address,city,suburb,state,country,zip FROM addressbook_table where user_id='".$userid."' and id='".$addressid."'";
		$queryUserDetails = new Bin_Query();
		$queryUserDetails->executeQuery($sqlUserDetails);
