...
/**
 * This class contains functions to display the product review details
 * at the admin side.
 *
 * @package  		Model_MAdminProductReview
 * @category  		Model
...
class Model_MAdminProductReview
{
	
	
	/**
	 * Function displays all the product review details at the admin side 
	 * 
	 * 
	 * @return array
	 */
	...
	/**
	 * Function displays a search criteria at the admin side 
	 * 
	 * 
	 * @return array
	 */	
	...
	{	
		...
		{
			include('classes/Core/CAdminProductReview.php');
			$output['adminreview'] = Core_CAdminProductReview::searchReviewDetails();
	...
	/**
	 * Function updates the status (Active Status) for a product at the admin side 
	 * 
	 * 
	 * @return array
	 */	
	...
	function reviewAcceptStatus()
	{
		...
		if($chkuser)
		{
			...
			$output['statusmsg'] = $default->acceptReview();
			$output['adminreview'] = Core_CAdminProductReview::showReviewDetails();
			Bin_Template::createTemplate('AdminProductReview.html',$output);
		}
		else
	...
	function reviewDenyStatus()
	{
		...
		if($chkuser)
		{
			include('classes/Core/CAdminProductReview.php');
			$default = new Core_CAdminProductReview();
			$output['statusmsg'] =  $default->denyReview();
			$output['adminreview'] = Core_CAdminProductReview::showReviewDetails();
			Bin_Template::createTemplate('AdminProductReview.html',$output);
		}
		else
	...
	function autoComplete()
	{
		...
		$default = new Core_CAdminProductReview();
		if($_GET['ids']==1)
			$output['username']=$default->autoComplete();
		elseif($_GET['ids']==2)
