...
class Lib_FormValidation extends Lib_Validation_Handler 
{
	...
	function validateSiteSettings()
	{
		...
		if($_FILES['site_favicon']['name']=='' && $_POST['site_favicon']=='' )
		{
			$this->Assign("site_favicon","","noempty"," Favicon -".$message);
		}
		...
		if($_FILES['site_favicon']['name'] != '')
		{
			...
			//Image Validation
			$ext_array = array('.jpeg','.jpg','.gif','.png','.bmp');
			$ext = strchr($_FILES['site_favicon']['name'],'.');
			if(!in_array(strtolower($ext),$ext_array))
				$this->Assign("site_favicon","","noempty","Favicon - ".$this->formatmessage);
			else
			{
				$img = getimagesize($_FILES['site_favicon']['tmp_name']);
				$mime = array('image/jpeg','image/jpg','image/png','image/gif');
